...
def test_confirm_is_five_fields_and_init_asks_about_each(tmp_path, monkeypatch):
    """`discover`'s docstring said "exactly one field is here". There are five,
    ...
    here first; `project.maintainer` joined on 2026-08-20 and did too.
    """
    repo = _project(tmp_path, board=False)
    (repo / "pkg" / "core").mkdir()
    (repo / "pkg" / "core" / "__init__.py").write_text("", encoding="utf-8")
    ...
    assert confirm == {"branches.integration", "memory.orientation", "layering.forbid",
                       "board.decision_attributor", "project.maintainer"}
