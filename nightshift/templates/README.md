...
| `ai/recipes/*.md` | `.ai/recipes/` | the three framework spines — two about *writing rules*, one about *writing tests* — never about any project's subject. They are what makes `recipe:` on a card resolvable at all on a fresh install |
...
| `{{maintainer}}` | `.ai/hosts.json`'s entry for this machine, then `[project].maintainer`, then `git config user.name` |
...
`{{maintainer}}` reads a declared name before git's because `user.name` is a
*commit identity* and is frequently a handle rather than a name — and this token is
how a charter addresses the person. It reads the **per-machine** file before the
project-wide field because identity belongs to a person: a project-wide field gives
two maintainers one answer between them, while `hosts.json` is already committed and
already keyed by hostname. That only started to matter once `update`
