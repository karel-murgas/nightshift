...
- **Never hard-wrap a card's prose across multiple source lines.** Most repos wrap their docs
  ...
  `{{board}}/`: cards are read and edited in Obsidian, whose editor shows each source line as
  its own row instead of reflowing the paragraph — and a `` `code` `` span that opens on one
  line and closes on a later one does not survive the split. CommonMark closes a code span at
  the next backtick regardless of the newline in between, so the wrapped span leaks unescaped
  text into the render and Obsidian stops rendering the rest of the file. Measured, not
  hypothetical: three spans wrapped this way in one card were caught by screenshot, not by
  anyone reading the raw file. Write every paragraph, bullet and picker option as **one
  continuous source line**, however long — let the editor soft-wrap it for display. This is a
  card-*writing* rule, not just an editing one: it binds `triage` and any worker appending
  `## Summary` / `## Thread` as much as it binds you.
