...
def _startup_housekeeping(ctrl: Path, work: Path) -> None:
    ...
    if swept := sweep_terminal_cards(work):
...
def run_lifecycle(ctrl: Path, base: str, *, kind: str, label: str,
    ...
    if not dry_run:
        _open_run_log(ctrl)
    check = preflight(ctrl, base, dry_run, named_card=named_card)
    if not check.ok:
        ...
        raise RunRefused(EXIT_STOPPED if (ctrl / STOP_FILE).is_file() else 1)

    if not dry_run and not acquire_lock(ctrl):
        raise RunRefused(1)
